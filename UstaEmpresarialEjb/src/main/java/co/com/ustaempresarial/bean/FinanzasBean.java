...
public class FinanzasBean implements FinanzasFachada {
	...
	public List<LibroMayor> listarLibroMayor() throws Exception {
		...
		Query q = em.createNamedQuery(LibroMayor.LISTAR_LIBRO_MAYOR);
	...
	/**
	 * Metodo que crea un plan contable. Y a su vez hacer el ingreso de la nueva
	 * cuenta en la tabla log de cuentas Cuentas
	 *
	...
	public List<LibroMayor> listarLibroMayorById(int id) throws Exception {
		List<LibroMayor> libroMayores = new ArrayList<>();
		Query q = em.createNamedQuery(LibroMayor.LISTAR_LIBRO_MAYOR_BYID);
		libroMayores = q.getResultList();
		if (libroMayores.size() > 0) {
			return libroMayores;
		}
		return null;
